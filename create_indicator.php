...
// === [ ส่วนที่ 2: ดึง Order_no ล่าสุด (ต้องมีส่วนนี้ ค่าถึงจะขึ้น) ] ===
    ...
    <div class="form-wrapper">
        ...
        <form action="config/checkcreate_indicator.php" method="POST" onsubmit="return confirm('คุณต้องการบันทึกข้อมูลใช่หรือไม่?');">
            ...
            <div class="form-group">
                ...
                <select name="KPI_type_id" id="kpi_type_select" data-loaded-type-id="<?php echo $loaded_kpi_type_id; ?>" required>
            ...
            <div class="form-group">
                ...
                <div>
                    <label style="display: inline-block; font-weight: normal; padding-top: 0; font-size: 1.3rem;">
                        <input type="radio" name="fill_data" value="no" style="width: auto; vertical-align: middle; margin-right: 5px;" checked> ไม่กรอกข้อมูล
                    </label>
                    <label style="display: inline-block; margin-right: 20px; font-weight: normal; padding-top: 0; font-size: 1.3rem;">
                        <input type="radio" name="fill_data" value="yes" style="width: auto; vertical-align: middle; margin-right: 5px;"> กรอกข้อมูล
                    </label>
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <div>
                    <label style="display: inline-block; font-weight: normal; padding-top: 0; font-size: 1.3rem;">
                        <input type="radio" name="fetch_data" value="no" style="width: auto; vertical-align: middle; margin-right: 5px;" checked> ไม่ดึงข้อมูล
                    </label>
                    <label style="display: inline-block; margin-right: 20px; font-weight: normal; padding-top: 0; font-size: 1.3rem;">
                        <input type="radio" name="fetch_data" value="yes" style="width: auto; vertical-align: middle; margin-right: 5px;" > ดึงข้อมูล
                    </label>
                </div>
            </div>
            ...
            <div class="form-group" id="article_type_group" style="display: none;">
                ...
                <select name="Table_id" id="article_type_select">
                    <?php
                    // ดึงข้อมูลจากตาราง tablename (ยกเว้น individual_kpi หรือ id 1 ถ้าไม่ต้องการ)
                    // ในที่นี้เลือกเฉพาะ ID 2 (research) และ 3 (publication) ตามโค้ดเดิม
                    ...
                    // ตรวจสอบว่ามีค่าเดิมหรือไม่ (กรณีแก้ไข/โหลดซ้ำ)
            ...
            <div class="form-group" id="publication_options_group" style="display: none;">
                ...
                <select name="Publication_type_id" id="publication_type_select">
                    <?php
                    // ดึงข้อมูลจากตาราง publicationtype
            ...
            <div class="form-group">
                ...
                <input type="number" name="Order_no" id="order_no_input" 
                       ...
                       data-loaded-order-no="<?php echo isset($row['Order_no']) ? $row['Order_no'] : ''; ?>" 
                       required>
            </div>
            <div class="form-group">
                <label>ชื่อตัวชี้วัด :</label> 
                <input type="text" name="KPI_topic_name" value="<?php echo isset($row['KPI_topic_name']) ? htmlspecialchars($row['KPI_topic_name']) : ''; ?>" required>
            </div>
            <div class="form-group">
                <label>หน่วยวัด :</label>
                <input type="text" name="Unit" value="<?php echo isset($row['Unit']) ? htmlspecialchars($row['Unit']) : ''; ?>">
            </div>
            <div class="form-group">
                <label>เป้าหมายความสำเร็จที่คาดหวังทั้งปี :</label>
                <input type="text" name="Goal" value="<?php echo isset($row['Goal']) ? htmlspecialchars($row['Goal']) : ''; ?>" >
            </div>
            
            <div class="form-group">
                <label>เกณฑ์การให้คะแนนตามผลงานที่ทำได้ :</label>
                <textarea name="Score_criteria" rows="5" ><?php echo isset($row['Score_criteria']) ? htmlspecialchars($row['Score_criteria']) : ''; ?></textarea>
            </div>
            <div class="form-group">
                <label>น้ำหนัก :</label>
                <input type="number" step= "0.5" name="Weight" value="<?php echo isset($row['Weight']) ? $row['Weight'] : ''; ?>" required>
            </div>

            <div class="form-group">
                <label>หมายเหตุ :</label>
                <input type="text" name="Description_text" value="<?php echo isset($row['Description_text']) ? htmlspecialchars($row['Description_text']) : ''; ?>">
            </div>
