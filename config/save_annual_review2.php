...
$sql_upsert_individual = "INSERT INTO individual_kpi
    ...
    foreach ($data as $topic_id => $topic_data) {
        ...
        // ตรวจสอบว่ามีการอัปโหลดไฟล์สำหรับ Topic นี้หรือไม่ (ใช้ ...['name'][0] สำหรับ multi-upload)
        ...
        mysqli_stmt_bind_param(
            ...
            $goal_h2,
            $actual_h2,
            $actual_work_all_year_value, // ใช้ค่าที่คำนวณใหม่
            $score_value,
            $total_score_value,
    ...
    // --- 3. Update/Insert total_score_evaluation ---
    if ($all_queries_success) {
